...
document.querySelector('.product-remove').addEventListener('click', (event) => {
    localStorage.productsInCart = JSON.stringify(JSON.parse((localStorage.getItem('productsInCart')).filter(item => item === event.target.parentElement)));
})
